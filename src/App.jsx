...
import './index.css';
import staticParkingsData from './data/static_parkings.json';
...
function App() {
    ...
    // 預先處理靜態資料 (加上 type='static')
    // 注意：ID 必須要跟 TDX API 回傳的 CarParkID 一致才能正確整合
    const staticParkings = staticParkingsData.map(p => ({
        ...p,
        available: -1,
        type: 'static'
    }));
    ...
    const loadData = async (lat, lng, city, isSilent = false) => {
        ...
        try {
            // 1. 準備基礎資料：從 Static List 中找出距離較近的
            // 由於全台資料量大，這裡先做前端距離過濾 (20km)
            const nearbyStatic = staticParkings.map(p => {
            ...
            // 3. 合併資料：將即時狀態填入靜態資料
            const mergedData = nearbyStatic.map(p => {
                // 嘗試用 ID 匹配動態資料
                if (statusMap[p.id] !== undefined) {
                    return {
                        ...p,
                        available: statusMap[p.id],
                        type: 'dynamic' // 標記為有動態資料
                    };
                }
                return p; // 保持靜態
            });
    ...
    const updateMapMarkers = (data) => {
        ...
        const markers = data.map(p => {
            let colorClass = 'marker-blue'; // 預設靜態藍
            let availableText = 'P';

            if (p.type === 'dynamic' && p.available !== -1) {
            ...
            const popupContent = `
                <div class="font-sans min-w-[200px]">
                    <h3 class="font-bold text-sky-600 text-lg mb-1">
                        ${p.name} 
                        ${p.type === 'static' ? '<span class="text-xs bg-slate-200 text-slate-600 px-1 rounded ml-1">資訊</span>' : ''}
                    </h3>
                    <div class="flex justify-between items-center mb-2">
                        <span class="text-slate-500 text-sm">總車位: ${p.total || '--'}</span>
                        <span class="text-2xl font-bold ${colorClass.replace('marker-', 'text-')}">
                            ${p.type === 'static' ? '--' : (p.available === -1 ? '未知' : p.available)}
                        </span>
                    </div>
                    ${p.type === 'static' ? '<div class="text-xs text-slate-500 mb-2">無即時剩餘車位資料</div>' : ''}
                    <div class="text-xs text-slate-500 mb-2">${p.fare || p.address || ''}</div>
                ...
                <div className="absolute inset-0 bg-slate-100 overflow-y-auto pb-32">
                    <div className="p-4 pt-24 max-w-md mx-auto space-y-4">
                        {parkings.map((p) => {
                            ...
                            if (p.type === 'dynamic' && p.available !== -1) {
                                displayText = p.available;
                                if (p.available === 0 || p.available / p.total < 0.1) statusColor = 'text-red-500';
                                else if (p.available / p.total < 0.3) statusColor = 'text-yellow-500';
                            ...
                            return (
                                <div key={p.id} className="glass-panel p-4 rounded-xl border-l-4 border-sky-500 transform transition hover:scale-[1.02] bg-white shadow-sm">
                                    <div className="flex justify-between items-start">
                                        <div>
                                            <h3 className="text-lg font-bold text-slate-800">
                                                {p.name}
                                                {p.type === 'static' && <span className="ml-2 text-xs bg-slate-100 text-slate-500 px-2 py-0.5 rounded-full">資訊</span>}
                                            </h3>
                                    ...
                                    <p className="text-xs text-slate-500 mt-2 line-clamp-1">{p.fare || p.address}</p>
                                    <button
                        ...
                        {parkings.length === 0 && !loading && (
                            <div className="text-center text-slate-500 mt-10">附近 20 公里內無資料</div>
                        )}
                    </div>
                </div>
            )}
                ...
                <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
                    <div className="glass-panel max-w-sm w-full rounded-2xl p-6 border border-white shadow-2xl relative bg-white/95">
                        ...
                        <div className="space-y-3 text-sm text-slate-600">
                            ...
                            <div className="flex items-start gap-3">
                                <div className="w-8 h-8 rounded-full bg-blue-500 flex items-center justify-center text-xs font-bold text-white shadow-sm">
                                    P
                                </div>
                                <p className="mt-1.5"><span className="text-blue-500 font-bold">藍色泡泡</span>：靜態資訊(無即時數據)。</p>
                            </div>
